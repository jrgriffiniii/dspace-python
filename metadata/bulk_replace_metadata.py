...
def check_collection_in_csv(csv_file, collection):
    ...
    for l in csv_file.readlines():
        ...
        if (col != 'collection' and collection not in  col): 
            raise Exception(
                "{} contains entry for collection '{}', expecting entries for '{}' only".format(csv_file.name, col,
                                                                                                collection))
    csv_file.seek(0)
...
def export_metadata(collection):
    ...
    cmd = "{}/bin/dspace metadata-export --all -f {} -i {}".format(__DSPACE_HOME__, bak_file, collection)
    systm(cmd)
...
def import_metadata(csv_file, collection):
    ...
    cmd = "{}/bin/dspace metadata-import --silent -f {}".format(__DSPACE_HOME__, csv_file.name)
    try:
        systm(cmd)
...
def col_file(collection, ext):
    ...
    mkdirpath(dir_name)
    bak_file = "{}/{}-{}-{}.csv".format(dir_name, collection.replace('/', '-'), __RUN_ID__, ext)
    return bak_file
...
"""
    ...
    parser.add_argument('--solr',  action='store_true', help='optimize solr index after import')
