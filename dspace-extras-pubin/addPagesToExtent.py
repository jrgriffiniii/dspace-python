...
DSPACE_LIST = " bulk-list --root ROOT --class ITEM " +  \
    ...
    if (options != None): 
        ...
        for line in output.split("\n"): 
            ...
            if (not line.startswith('ITEM')):  
                continue; # ignore anything that is not related to items 
            ...
            try: 
               if (not item[options.metaElem]):  
                    ...
                    continue;
               if (type(item[options.metaElem]) ==  list) and (len(item[options.metaElem]) == 1):  
                    item[options.metaElem] = item[options.metaElem][0];
               elif (type(item[options.metaElem]) !=  str):  
                    doFinalLog("NON-STRING-VALUE" + str(type(item[options.metaElem])), item, options); 
                    continue; 
