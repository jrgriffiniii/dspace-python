...
#     openpyxl.load_workbook(filename='Thesis.xlsx')
...
read thesis submission info from file given in --thesis option
 if --add_certs option is given read info about additional certificate programs
 from file generate certificate program specific tsv files with submissions to
 the relevant program home department submissions that have bo certificate
 program desifgnation are printed to the file None.tsv
"""
        ...
        parser.add_argument(
            "--thesis", "-t",
            default=None, required=True, help="excel export file from vireo")
        parser.add_argument(
            "--split_col", "-s",
            required=False, default=VireoSheet.CERTIFICATE_PROGRAM,
            help="split column name - default %s" % VireoSheet.CERTIFICATE_PROGRAM
        )
        parser.add_argument(
            "--add_certs", "-a",
        ...
        parser.add_argument(
            "--all_cols", "-A", action='store_true',
            help="include all columns in printout"
        )
        parser.add_argument(
            "--loglevel", "-l", choices=loglevels,  default=logging.INFO,
...
PRINT_COLUMNS = [VireoSheet.CERTIFICATE_PROGRAM, VireoSheet.STUDENT_NAME,
                 ...
                 VireoSheet.ADVISORS, VireoSheet.DOCUMENT_TITLE,
                 VireoSheet.PRIMARY_DOCUMENT]


def addToSplits(splits, split_on, split_data, submissions):
    logging.info(
        "addToSplits %s data=%s submissions=%s" % (
            split_on, split_data.file_name, submissions.file_name
        )
    )
    ...
    for id in split_data.id_rows:
        ...
        for data_row in split_data.id_rows[id]:
            ...
            # TODO: linter insists on "not in" but I don't think that's the
            #   same logic
...
def _select_row_values(row, col_ids):
    ...
    return [(str(c.value) if c.value is not None else 'None') for c in sel_row]
...
def main():
    try:
        ...
        splits = {}
        addToSplits(
            splits, split_on=args.split_col, split_data=submissions,
        ...
        if (args.add_certs):
            ...
            addToSplits(
                splits, split_on=args.split_col, split_data=moreCerts,
                submissions=submissions
            )
