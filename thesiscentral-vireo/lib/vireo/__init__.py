...
class VireoSheet:
    ...
    def col_index_of(self, col_header, required=False):
        ...
        try:
            ...
            if required:
                raise Exception(
                    "%s does not contain a '%s' column" % (self.file_name, col_header)
                )
        return None
    ...
    def _derive_id_hash(self):
        ...
        for row in self._sheet.iter_rows(min_row=2):
            ...
            if not cell.value:
                ...
                logging.warning(
                    "%s: Row has no ID value: %s" % (self.file_name, first_field)
                )
            else:
                ...
                if not row_id in id_rows:
                    ...
                    raise Exception(
                        "%s has duplicate id %s" % (self.file_name, str(row_id))
                    )
                else:
    ...
    def readMoreCerts(self, add_certs_file, check_id=True):
        ...
        for cert_id in add_certs.id_rows:
            ...
            for row in add_certs.id_rows[cert_id]:
                logging.debug(
                    "ID %d -> cert_row: %s" % (cert_id, VireoSheet.row_values(row))
                )
                # check that student email matches
                if (
                    row[certs_email_col_id].value.strip()
                    != thesis_row[thesis_email_col_id].value.strip()
                ):
                    raise Exception(
    ...
    def log_info(self):
        ...
        logging.info(
            "%s: headers %s" % (self.file_name, str.join(",", self.col_names()))
        )
        for col in [VireoSheet.STUDENT_EMAIL, VireoSheet.CERTIFICATE_PROGRAM]:
            logging.info(
                "%s column: %s (%s)"
                % (self.file_name, col, str(self.col_index_of(col)))
            )
        logging.info("---")

    def __str__(self):
        return "%s: ID%s-col:%d, nrows:%d" % (
            self.file_name,
            ("(unique)" if self.unique_ids else ""),
            self.id_col,
            len(self.id_rows),
        )
