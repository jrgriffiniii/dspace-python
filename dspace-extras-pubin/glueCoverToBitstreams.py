...
DSPACE_OPTS = " --doWorkFlowItems  --include object,ITEM.id,internalId,BUNDLE.name,BUNDLE.id,mimeType,storeNum,checksum,checksumAlgo," + \
    ...
    if (options != None): 
        ...
        for line in output.split("\n"): 
            if (line[0] == '#' or line.startswith("INFO") or line.startswith("DEBUG")):
                ...
                continue;  # skip comment and log lines
            ...
            try: 
               ...
               bitstream['fileName'] = getDSpaceFileName(options.assetstores, bitstream['storeNum'], bitstream['internalId']);
               bitstream['pdfFileName'] = options.bitstream_covered_dir + "/" + \
                                          "BUNDLE."  + bitstream['BUNDLE.id'] +":" + bitstream['name']
               bitstream['pdfFileName'] = re.sub("[(){}$ '\"\[\]]", "", bitstream['pdfFileName']);
               bitstream['md5FileName'] = os.path.splitext(bitstream['pdfFileName'])[0] + ".md5";
               doLog(bitstream['md5FileName'] + " checkMetaData " + bitstream['ITEM.id'], options.log_file) 
               ...
               cmd = options.dspace_cmd + " " + DSPACE_METADATA.replace("ITEM", "ITEM." + bitstream['ITEM.id']); 
...
def parseargs(): 
    parser = OptionParser()
    parser.add_option("-a", "--assetstores", dest="assetstores",
    ...
    try: 
        ...
        options.assetstores = ["%s/%s" %(options.dhome, a) for a in options.assetstores.split(",")]
        ...
        options.pdfAddCoverCmd = "gs -dBATCH -dNOPAUSE -q -sDEVICE=pdfwrite -sOutputFile=BITSTREAM %s ASSETSTOREFILE" % (options.cover)
