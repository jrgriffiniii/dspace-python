...
DSPACE_LIST = (
    " bulk-list --root ROOT --class ITEM "
    + " --include object,DSPACE_METADATA_ELEM"
    + " --format TSV "
)
DSPACE_METADATA = (
    " bulk-meta-data --root ROOT " + " --eperson EPERSON " + " --format TSV "
)
    ...
    if options != None:
        ...
        for line in output.split("\n"):
            ...
            if not line.startswith("ITEM"):
                continue
                # ignore anything that is not related to items
            ...
            try:
                if not item[options.metaElem]:
                    ...
                    continue
                if (type(item[options.metaElem]) == list) and (
                    len(item[options.metaElem]) == 1
                ):
                    item[options.metaElem] = item[options.metaElem][0]
                elif type(item[options.metaElem]) != str:
                    doFinalLog(
                        "NON-STRING-VALUE" + str(type(item[options.metaElem])),
                        item,
                        options,
                    )
                    continue
...
def setField(item, options):
    try:
        ...
        # ADD  new value
        addcmd = cmd + " -a ADD -m '%s=%s'" % (
            options.metaElem,
            item[options.metaElem] + DSPACE_APPEND_VALUE,
        )
...
def parseargs():
    ...
    parser.add_option(
        ...
        "--pattern",
        dest="valuePattern",
        help="Pattern to match against metadata value, default "
        + DSPACE_METADATA_PATTERN,
    )
    parser.add_option(
        "-r",
        "--root",
