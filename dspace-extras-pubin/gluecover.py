...
def doit():
    ...
    cmdTemplate = "pdftk %s %s/BITSTREAM output %s/BITSTREAM" % (
        options.cover,
        options.indir,
        options.outdir,
    )
...
def parseargs():
    parser = OptionParser()
    parser.add_option("-i", "--in", dest="indir", help="directory containing pdf files")
    parser.add_option(
        "-o",
        "--out",
        dest="outdir",
    ...
    parser.add_option("-c", "--cover", dest="cover", help="Cover page pdf")
    parser.add_option(
        "-v",
        "--verbose",
        action="store_true",
