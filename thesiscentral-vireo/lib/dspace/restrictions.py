...
def choose(matches, vireo, col_name, title_col_name, logger):
    ...
    for i, matching_row in enumerate(matches):
        try:
            ...
            logger.info(
                "Choice {}: {} ({})".format(
                    i + 1, option_value.strip(), title_cell_value
                )
            )
...
def save(vireo, logger):
    ...
    try:
        ...
        default_path = os.path.abspath(
            os.path.join(parent_directory, "export", "ImportRestrictions.xlsx")
        )

        logger.info(
            "Saving the updated authorizations files to {}".format(default_path)
        )
...
def matchIds(submissions, restrictions, logger):
    ...
    s_col_ids = [
        submissions.col_index_of(col)
        for col in [VireoSheet.STUDENT_NAME, VireoSheet.DOCUMENT_TITLE]
    ]
    ...
    for row in rows:
        ...
        logger.debug(
            "Processing restriction record {} ({})...".format(title_value, r_name)
        )
        ...
        if submission_id is None:
            ...
            if len(matches) > 0 and matching_row != None:
                ...
                logger.debug(
                    "No ID matches were found for the author {}".format(author_name)
                )
        else:
            ...
            logger.debug(
                "The IDs {} were matched...".format(matching_id_values.join("\n"))
            )
